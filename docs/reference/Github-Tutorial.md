...
This tutorial will guide you step-by-step through the basics of using GitHub with JupyterHub. There are **two methods** for performing Git operations:

1. **Using the Terminal**: Ideal for users who prefer command-line control and advanced flexibility.
1. **Using the JupyterHub Git Interface**: A user-friendly interface for performing common Git tasks visually.
...
- Select the scopes you need (e.g., repo for full repository access).
...
- Open the terminal in JupyterHub (`File -> New -> Terminal`).

- Go to the GitHub repository you want to clone, click **Code**, and copy the HTTPS link.

- Use the following command to clone the repository:

    ```bash
    git clone https://<your-username>:<your-token>@github.com/<owner>/<repository>.git
    ```
...
- In the top-right corner of the GitHub page, click the **+** icon.
- Select **New repository** from the dropdown menu.

#### Step 3: Set Up the Repository

- **Choose a repository name**: Enter a unique name for your repository. For example, `my-new-repo`.
- **Add a description (optional)**: Write a short description of your project.
...
- Open the **Git** tab in JupyterHub (usually in the right-hand sidebar).
- Click **Clone Repository**.
- Paste the HTTPS link of the repository and confirm. The repository will be cloned into your current working directory.
...
- Initialize a Git repository in JupyterHub:
...
- **Developer A's Change**: Developer A updates the file and pushes it:

    ```text
    Hello GitHub
    ```

- **Developer B's Change**: Without pulling the latest changes, Developer B updates the same file:

    ```text
    Hello Git
    ```

- **The Conflict**: When Developer B tries to push their changes, Git rejects the push and provides the following error message:
...
- **Resolving the Conflict**: Developer B edits the `example.txt` file to keep the desired version, for example:

    ```text
    Hello Git and GitHub
    ```

- **Final Steps**: Developer B stages the resolved file and completes the merge:
...
- **If the file has changed in the remote repository**:
...
- **If the file has not changed in the remote repository**:
...
```bash
(notebook) jovyan@jupyter-username:~/project1$ git remote add origin https://<your-username>:<your-token>@github.com/<owner>/<repository>.git
```
